...
@interface BATCourseReplyTableView () <UITableViewDelegate,UITableViewDataSource>
...
@implementation BATCourseReplyTableView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self.tableView registerClass:[BATCourseReplyCommentCell class] forCellReuseIdentifier:@"BATCourseReplyCommentCell"];
    }
    return self;
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_dataSource.count > 0) {
        BATCourseCommentData *comment = _dataSource[indexPath.row];
...
{
    BATCourseReplyCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:@"BATCourseReplyCommentCell" forIndexPath:indexPath];
    if (_dataSource.count > 0) {
        BATCourseCommentData *comment = _dataSource[indexPath.row];
