//
//  KMDEditImageViewController.m
//  KMDance
//
//  Created by Skybrim on 2017/11/23.
...
@interface KMDEditImageViewController ()
{
...
@implementation KMDEditImageViewController
...
//    CGSize size = CGSizeMake(width*scale, width*scale*self.oriImage.size.height/self.oriImage.size.width);
    
//    [_indicator startAnimating];
    
    CGFloat W = 70;
    CGSize  imgSize = self.oriImage.size;
    CGFloat maxW = MIN(imgSize.width, imgSize.height);
    UIImage *iconImage = [self scaleImage:self.oriImage toSize:CGSizeMake(W * imgSize.width/maxW, W * imgSize.height/maxW)];
    for (UIView *v in self->_menuScroll.subviews) {
...
- (void)btnDone_click
{
    ...
    if (self.editImageBlock) {
        self.editImageBlock([self clipImage]);
    }
